        ...
        <li class="p-4 hover:bg-gray-100 cursor-pointer flex items-center space-x-4">
            ...
            <div class="flex justify-between items-center w-full">
                <div>
                    ...
                    <span class="text-xs text-gray-500">By: {{ $notification->user->name }}</span>

                    <!-- Conditional Message Based on User Type -->
                    @if (Auth::user()->usertype === 'admin')
                    <span class="text-xs text-red-500 font-semibold">(Admin Only)</span>
                    @elseif (Auth::user()->usertype === 'dept_head')
                    <span class="text-xs text-blue-500 font-semibold">(Department Head Only)</span>
                    @endif
                </div>
